 ...
 */
public class PlanificacionMaleta_v2 {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // [FIX-2 + FIX-3 + FIX-4]  ESTADO COMPARTIDO DE CAPACIDAD DE VUELOS
    // ═══════════════════════════════════════════════════════════════════════

    /**
     * Registra cuántas maletas lleva cada INSTANCIA DIARIA de vuelo.
     *
     * [FIX-4] La clave incluye la fecha UTC de salida: "ORIG-DEST-HH:MM-YYYY-MM-DD".
     * Sin la fecha, el vuelo SBBR→SGAS 02:00 del día 2 y del día 3 compartían
     * una única capacidad de 320 maletas para toda la simulación, provocando
     * FAILs incorrectos aunque hubiera cupo real en días posteriores.
     *
     * La entrada se crea bajo demanda (lazy) la primera vez que A* encuentra
     * ese slot en ese día concreto.
     */
    static class EstadoVuelos {
        // "ORIG-DEST-HH:MM-YYYY-MM-DD" → [ocupado, cap_max]
        private final ConcurrentHashMap<String, int[]> estado = new ConcurrentHashMap<>();

        EstadoVuelos() {}

        /** Clave única para una instancia diaria de vuelo. */
        private String clave(Vuelo v, LocalDate fechaUtc) {
            return v.origen + "-" + v.destino + "-"
                 + FMT_TIME.format(v.salidaLocal) + "-"
                 + FMT_DATE.format(fechaUtc);
        }

        /** Obtiene o crea la entrada para este slot diario. */
        private int[] getOCrear(Vuelo v, LocalDate fechaUtc) {
            String k = clave(v, fechaUtc);
            return estado.computeIfAbsent(k, x -> new int[]{0, v.capacidad});
        }

        boolean hayCapacidad(Vuelo v, LocalDate fechaUtc, int cantidad) {
            int[] e = getOCrear(v, fechaUtc);
            return (e[0] + cantidad) <= e[1];
        }

        /** Reserva atómica usando synchronized en la entrada del mapa. */
        boolean reservar(Vuelo v, LocalDate fechaUtc, int cantidad) {
            int[] e = getOCrear(v, fechaUtc);
            synchronized (e) {
        ...
        void liberar(Vuelo v, LocalDate fechaUtc, int cantidad) {
            String k = clave(v, fechaUtc);
            int[] e = estado.get(k);
            if (e != null) {
                synchronized (e) {
        ...
        int[] ocupacion(Vuelo v, LocalDate fechaUtc) {
            String k = clave(v, fechaUtc);
            int[] e = estado.get(k);
            return e != null ? new int[]{e[0], e[1]} : new int[]{0, v.capacidad};
        }
    }
    ...
    static class ResultadoAstar {
        ...
        final double         costoHoras;
        final List<Object[]> reservasHechas;  // Object[]{Vuelo, LocalDate}

        ResultadoAstar(List<String> ruta, List<Vuelo> vuelosUsados,
                       double costoHoras, List<Object[]> reservasHechas) {
            this.ruta           = ruta;
    ...
    //     // [FIX-4] reservas almacena (vuelo, fechaUtc) para rollback correcto
    ...
    //             // [FIX-2 + FIX-4] Capacidad del vuelo para ESTE DÍA concreto
    //             LocalDate fechaVueloUtc = salUtc.toLocalDate();
    //             if (!estadoVuelos.hayCapacidad(vuelo, fechaVueloUtc, cantidad)) continue;
    ...
    //             // [FIX-3] Reserva atómica en el vuelo compartido (con fecha)
    //             if (!estadoVuelos.reservar(vuelo, fechaVueloUtc, cantidad)) continue;
    //             reservas.add(new Object[]{vuelo, fechaVueloUtc});
...
static ResultadoAstar aStar(
    ...
    Random rng = new Random(System.nanoTime()); // 👈 generador aleatorio
    ...
    while (!abiertos.isEmpty()) {
        ...
        // 🔀 Copia y mezcla de vecinos (aleatoriedad reproducible)
        ...
        for (Vuelo vuelo : vecinos) {
            ...
            abiertos.add(new Nodo(
                destVuelo,
                llegUtc,
                nuevoCosto + h + ruido, // 👈 aquí entra la aleatoriedad
                actual,
                vuelo
            ));
        }
    ...
    static Object planificarEnvio(
        ...
        if (res == null) {
            // Rollback: liberar reservas parciales con su fecha correcta
            for (Object[] r : res != null ? res.reservasHechas : reservas)
                estadoVuelos.liberar((Vuelo) r[0], (LocalDate) r[1], envio.cantidad);
        ...
        // Snapshot de ocupación para el reporte (usando la fecha del vuelo desde reservas)
        int[] ocup = new int[res.vuelosUsados.size()];
        for (int i = 0; i < res.vuelosUsados.size() && i < res.reservasHechas.size(); i++) {
            Vuelo v = res.vuelosUsados.get(i);
            LocalDate fd = (LocalDate) res.reservasHechas.get(i)[1];
            ocup[i] = estadoVuelos.ocupacion(v, fd)[0];
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        int diasSim        = Integer.parseInt(params.getOrDefault("dias", "7"));
        ...
        // Estado compartido de vuelos (entradas creadas lazy por fecha UTC)
        EstadoVuelos estadoVuelos = new EstadoVuelos();
