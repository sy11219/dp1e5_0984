...
// Función auxiliar para calcular hora absoluta de salida del primer vuelo
function getFirstFlightDepartureMinute(
  ...
  if (requestMinuteOfDay <= departureMinute) {
    return requestDay * 1440 + departureMinute;
  } else {
    return (requestDay + 1) * 1440 + departureMinute;
  }
}
      ...
      <div className="table">
        {paginatedItems.length === 0 ? (
          ...
          paginatedItems.map(s => {
            ...
            const statusColor = 
              shipmentState === "Entregado" ? "#2f855a" :
              shipmentState === "En curso" ? "#3182ce" :
              shipmentState === "Planeado" ? "#ffbf00" :
              "#718096";
      ...
      {filtered.length > 0 && (
        <div className="segmented" style={{ marginTop: "0.75rem", justifyContent: "space-between" }}>
          ...
          <span className="text-sm">
            {currentPage}/{totalPages} · {filtered.length} envíos
          </span>
          <button 
            type="button" 
